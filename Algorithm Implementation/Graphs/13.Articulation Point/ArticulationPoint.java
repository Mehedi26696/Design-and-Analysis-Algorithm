...
public class ArticulationPoint {
    ...
    public static void dfs(ArrayList<Edge> graph[], int curr, int par, int disc[], int low[], int time,
        ...
        for (Edge edge : graph[curr]) {
            ...
            if (neigh == par) {
                ...
                if (par != -1 && disc[curr] <= low[neigh]) {
                    ...
                    isArticulation[curr] = true; // if the current node is not the root node and the discovery time of
                                                 // the current node is less than or equal to
                    // the lowest discovery time of the neighbour, then the current node is an
                    // articulation point
                }
        ...
        if (par == -1 && children > 1) {
            isArticulation[curr] = true; // if the current node is the root node and it has more than one child, then
